...
const UnitForm = () => {
  ...
  async function onSubmit(values: z.infer<typeof categorySchema>) {
    ...
    try {
      setImgLoading(true);
      const response: any = await axios.post("/api/unit", values);
  ...
  return (
    <Box className="w-full space-y-3">
      <h5 className="text-lg font-bold">Add Unit (cm, kg, etc)</h5>
          ...
          <Form {...form}>
            <form className="space-y-3" onSubmit={form.handleSubmit(onSubmit)}>
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input placeholder="cm, kg, etc" {...field} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
              ...
              <Button type="submit" disabled={imgLoading} size="sm">
                <Spinner loading={imgLoading} /> Create unit
              </Button>
            </form>
          </Form>
        </div>
